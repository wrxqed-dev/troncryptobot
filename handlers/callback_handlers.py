...
async def back(event):
    ...
    text = (f'🚀 <b>Crypto Bot</b> — это бот-кошелёк для получения, отправки, покупки и хранения криптовалюты в Telegram.\n\n'
            f'О всех возможностях читайте в <a href="https://t.me/alexsenddnews">официальном канале</a>')
...
async def success_create_usdt_personal(event):
    ...
    text = (f"🏷 <b>Персональный чек</b>\n\n"
            ...
            f"Делая это, вы передаете средства в этом чеке безвозмездно, без гарантий получить что-то взамен. "
            f"Мошенники могут использовать это, чтобы получить ваши монеты\n\n"
            f"<b>Ссылка на чек</b>: <span class='tg-spoiler'>t.me/sendsummersBot?start={cheque_id}</span>")
