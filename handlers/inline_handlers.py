...
async def inline_send(event):
    ...
    if query.startswith('t_'):
        ...
        title = f"Чек на {amount_str} {currency} ({ton_price:.2f}USD)"
        ...
        result = event.builder.article(
            ...
            text=f"🚀 <b>Чек</b> на <b>{amount_str} {currency} ({ton_price:.2f}USD)</b>{user_info}{comment_text}",
...
async def create_cheque_inline(event):
    ...
    if query != "" and not query.startswith("t_"):
        ...
        if (currency in ['usdt', None]) and balance_usdt >= amount:
            ...
            text = f'🚀 <b>Чек</b> на <b>{amount_str} USDT ({amount_str}USD)</b>'
            if username:
                text += f' для {username}'
            if comment and username:
                text += f'\n\n💬 {comment}'
            ...
            results.append(event.builder.article(
                id=article_id,
                title=f'Чек на {amount_str} USDT ({amount_str}USD)',
                description=description,
                text=text,
                thumb=InputWebDocument(
                    url="https://i.imgur.com/m0SU0WN.png",
        ...
        if (currency in ['ton', None]) and balance_ton >= amount:
            ...
            text = f'🚀 <b>Чек</b> на <b>{amount_str} TON ({amount_in_usd:.2f}USD)</b>'
            if username:
                text += f' для {username}'
            if comment and username:
                text += f'\n\n💬 {comment}'
            ...
            results.append(event.builder.article(
                id=article_id,
                title=f'Чек на {amount_str} TON ({amount_in_usd:.2f}USD)',
                description=description,
                text=text,
                thumb=InputWebDocument(
                    url="https://i.imgur.com/m0SU0WN.png",
